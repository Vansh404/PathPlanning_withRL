#BUILDING THE ENVIRONMENT FOR OUR AUTONOMOUS AGENT TO EXPLORE
...
class Environment(tk.Tk, object):
    ...
    def build_environment(self):
        self.canvas_window = tk.Canvas(self,  bg='black',
        ...
        for column in range(0, env_width * pixels, pixels):
            ...
            self.canvas_window.create_line(x0, y0, x1, y1, fill='grey')
        ...
        self.obstacle1 = self.canvas_window.create_rectangle(
            ...
            outline='grey', fill='#FF0021')
        # Saving the coordinates of obstacle 1 according to the size of agent
        # In order to fit the coordinates of the agent
        self.coords_obstacle1 = [self.canvas_window.coords(self.obstacle1)[0] + 3,
         self.canvas_window.coords(self.obstacle1)[1] + 3,
         self.canvas_window.coords(self.obstacle1)[2] - 3,
         self.canvas_window.coords(self.obstacle1)[3] - 3]

        # Obstacle 2
        # Defining the center of obstacle 2
        obstacle2_center = self.o + np.array([pixels * 2, pixels * 2])
        # Building the obstacle 2
        self.obstacle2 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle2 = [self.canvas_window.coords(self.obstacle2)[0] + 3,
         self.canvas_window.coords(self.obstacle2)[1] + 3,
         self.canvas_window.coords(self.obstacle2)[2] - 3,
         self.canvas_window.coords(self.obstacle2)[3] - 3]

        # Obstacle 3
        # Defining the center of obstacle 3
        obstacle3_center = self.o + np.array([pixels * 3, pixels * 2])
        # Building the obstacle 3
        self.obstacle3 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle3 = [self.canvas_window.coords(self.obstacle3)[0] + 3,
         self.canvas_window.coords(self.obstacle3)[1] + 3,
         self.canvas_window.coords(self.obstacle3)[2] - 3,
         self.canvas_window.coords(self.obstacle3)[3] - 3]

        # Obstacle 4
        # Defining the center of obstacle 4
        obstacle4_center = self.o + np.array([pixels * 3, pixels * 3])
        # Building the obstacle 4
        self.obstacle4 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle4 = [self.canvas_window.coords(self.obstacle4)[0] + 3,
         self.canvas_window.coords(self.obstacle4)[1] + 3,
         self.canvas_window.coords(self.obstacle4)[2] - 3,
         self.canvas_window.coords(self.obstacle4)[3] - 3]

        # Obstacle 5
        # Defining the center of obstacle 5
        obstacle5_center = self.o + np.array([pixels * 4, pixels * 10])
        # Building the obstacle 5
        self.obstacle5 = self.canvas_window.create_rectangle(
        ...
        # Saving the coordinates of obstacle 2 according to the size of agent
        # In order to fit the coordinates of the agent
        self.coords_obstacle5 = [self.canvas_window.coords(self.obstacle5)[0] + 3,
         self.canvas_window.coords(self.obstacle5)[1] + 3,
         self.canvas_window.coords(self.obstacle5)[2] - 3,
         self.canvas_window.coords(self.obstacle5)[3] - 3]

        # Obstacle 6
        # Defining the center of obstacle 6
        obstacle6_center = self.o + np.array([pixels * 4, pixels * 11])
        # Building the obstacle 6
        self.obstacle6 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle6 = [self.canvas_window.coords(self.obstacle6)[0] + 3,
         self.canvas_window.coords(self.obstacle6)[1] + 3,
         self.canvas_window.coords(self.obstacle6)[2] - 3,
         self.canvas_window.coords(self.obstacle6)[3] - 3]

        # Obstacle 7
        # Defining the center of obstacle 7
        obstacle7_center = self.o + np.array([pixels * 4, pixels * 12])
        # Building the obstacle 7
        self.obstacle7 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle7 = [self.canvas_window.coords(self.obstacle7)[0] + 3,
         self.canvas_window.coords(self.obstacle7)[1] + 3,
         self.canvas_window.coords(self.obstacle7)[2] - 3,
         self.canvas_window.coords(self.obstacle7)[3] - 3]

        # Obstacle 8
        # Defining the center of obstacle 8
        obstacle8_center = self.o + np.array([pixels * 5, pixels * 12])
        # Building the obstacle 8
        self.obstacle8 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle8 = [self.canvas_window.coords(self.obstacle8)[0] + 3,
         self.canvas_window.coords(self.obstacle8)[1] + 3,
         self.canvas_window.coords(self.obstacle8)[2] - 3,
         self.canvas_window.coords(self.obstacle8)[3] - 3]

        # Obstacle 9
        # Defining the center of obstacle 9
        obstacle9_center = self.o + np.array([pixels * 6, pixels * 12])
        # Building the obstacle 9
        self.obstacle9 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle9 = [self.canvas_window.coords(self.obstacle9)[0] + 3,
         self.canvas_window.coords(self.obstacle9)[1] + 3,
         self.canvas_window.coords(self.obstacle9)[2] - 3,
         self.canvas_window.coords(self.obstacle9)[3] - 3]

        # Obstacle 10
        # Defining the center of obstacle 10
        obstacle10_center = self.o + np.array([pixels * 2, pixels * 18])
        # Building the obstacle 10
        self.obstacle10 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle10 = [self.canvas_window.coords(self.obstacle10)[0] + 3,
         self.canvas_window.coords(self.obstacle10)[1] + 3,
         self.canvas_window.coords(self.obstacle10)[2] - 3,
         self.canvas_window.coords(self.obstacle10)[3] - 3]

        # Obstacle 11
        # Defining the center of obstacle 11
        obstacle11_center = self.o + np.array([pixels * 3, pixels * 18])
        # Building the obstacle 11
        self.obstacle11 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle11 = [self.canvas_window.coords(self.obstacle11)[0] + 3,
         self.canvas_window.coords(self.obstacle11)[1] + 3,
         self.canvas_window.coords(self.obstacle11)[2] - 3,
         self.canvas_window.coords(self.obstacle11)[3] - 3]

        # Obstacle 12
        # Defining the center of obstacle 12
        obstacle12_center = self.o + np.array([pixels * 4, pixels * 18])
        # Building the obstacle 12
        self.obstacle12 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle12 = [self.canvas_window.coords(self.obstacle12)[0] + 3,
         self.canvas_window.coords(self.obstacle12)[1] + 3,
         self.canvas_window.coords(self.obstacle12)[2] - 3,
         self.canvas_window.coords(self.obstacle12)[3] - 3]

        # Obstacle 13
        # Defining the center of obstacle 13
        obstacle13_center = self.o + np.array([pixels * 3, pixels * 19])
        # Building the obstacle 13
        self.obstacle13 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle13 = [self.canvas_window.coords(self.obstacle13)[0] + 3,
         self.canvas_window.coords(self.obstacle13)[1] + 3,
         self.canvas_window.coords(self.obstacle13)[2] - 3,
         self.canvas_window.coords(self.obstacle13)[3] - 3]

        # Obstacle 14
        # Defining the center of obstacle 14
        obstacle14_center = self.o + np.array([pixels * 3, pixels * 20])
        # Building the obstacle 14
        self.obstacle14 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle14 = [self.canvas_window.coords(self.obstacle14)[0] + 3,
         self.canvas_window.coords(self.obstacle14)[1] + 3,
         self.canvas_window.coords(self.obstacle14)[2] - 3,
         self.canvas_window.coords(self.obstacle14)[3] - 3]

        # Obstacle 15
        # Defining the center of obstacle 15
        obstacle15_center = self.o + np.array([pixels * 3, pixels * 21])
        # Building the obstacle 15
        self.obstacle15 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle15 = [self.canvas_window.coords(self.obstacle15)[0] + 3,
         self.canvas_window.coords(self.obstacle15)[1] + 3,
         self.canvas_window.coords(self.obstacle15)[2] - 3,
         self.canvas_window.coords(self.obstacle15)[3] - 3]

        # Obstacle 16
        # Defining the center of obstacle 16
        obstacle16_center = self.o + np.array([pixels * 10, pixels * 22])
        # Building the obstacle 16
        self.obstacle16 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle16 = [self.canvas_window.coords(self.obstacle16)[0] + 3,
         self.canvas_window.coords(self.obstacle16)[1] + 3,
         self.canvas_window.coords(self.obstacle16)[2] - 3,
         self.canvas_window.coords(self.obstacle16)[3] - 3]

        # Obstacle 17
        # Defining the center of obstacle 17
        obstacle17_center = self.o + np.array([pixels * 11, pixels * 15])
        # Building the obstacle 17
        self.obstacle17 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle17 = [self.canvas_window.coords(self.obstacle17)[0] + 3,
         self.canvas_window.coords(self.obstacle17)[1] + 3,
         self.canvas_window.coords(self.obstacle17)[2] - 3,
         self.canvas_window.coords(self.obstacle17)[3] - 3]

        # Obstacle 18
        # Defining the center of obstacle 18
        obstacle18_center = self.o + np.array([pixels * 12, pixels * 15])
        # Building the obstacle 18
        self.obstacle18 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle18 = [self.canvas_window.coords(self.obstacle18)[0] + 3,
         self.canvas_window.coords(self.obstacle18)[1] + 3,
         self.canvas_window.coords(self.obstacle18)[2] - 3,
         self.canvas_window.coords(self.obstacle18)[3] - 3]

        # Obstacle 19
        # Defining the center of obstacle 19
        obstacle19_center = self.o + np.array([pixels * 13, pixels * 15])
        # Building the obstacle 19
        self.obstacle19 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle19 = [self.canvas_window.coords(self.obstacle19)[0] + 3,
         self.canvas_window.coords(self.obstacle19)[1] + 3,
         self.canvas_window.coords(self.obstacle19)[2] - 3,
         self.canvas_window.coords(self.obstacle19)[3] - 3]

        # Obstacle 20
        # Defining the center of obstacle 2
        obstacle20_center = self.o + np.array([pixels * 13, pixels * 14])
        # Building the obstacle 20
        self.obstacle20 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle20 = [self.canvas_window.coords(self.obstacle20)[0] + 3,
         self.canvas_window.coords(self.obstacle20)[1] + 3,
         self.canvas_window.coords(self.obstacle20)[2] - 3,
         self.canvas_window.coords(self.obstacle20)[3] - 3]

        # Obstacle 21
        # Defining the center of obstacle 21
        obstacle21_center = self.o + np.array([pixels * 13, pixels * 13])
        # Building the obstacle 21
        self.obstacle21 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle21 = [self.canvas_window.coords(self.obstacle21)[0] + 3,
         self.canvas_window.coords(self.obstacle21)[1] + 3,
         self.canvas_window.coords(self.obstacle21)[2] - 3,
         self.canvas_window.coords(self.obstacle21)[3] - 3]

        # Obstacle 22
        # Defining the center of obstacle 22
        obstacle22_center = self.o + np.array([pixels * 21, pixels * 22])
        # Building the obstacle 22
        self.obstacle22 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle22 = [self.canvas_window.coords(self.obstacle22)[0] + 3,
         self.canvas_window.coords(self.obstacle22)[1] + 3,
         self.canvas_window.coords(self.obstacle22)[2] - 3,
         self.canvas_window.coords(self.obstacle22)[3] - 3]

        # Obstacle 23
        # Defining the center of obstacle 23
        obstacle23_center = self.o + np.array([pixels * 20, pixels * 22])
        # Building the obstacle 23
        self.obstacle23 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle23 = [self.canvas_window.coords(self.obstacle23)[0] + 3,
         self.canvas_window.coords(self.obstacle23)[1] + 3,
         self.canvas_window.coords(self.obstacle23)[2] - 3,
         self.canvas_window.coords(self.obstacle23)[3] - 3]

        # Obstacle 24
        # Defining the center of obstacle 24
        obstacle24_center = self.o + np.array([pixels * 19, pixels * 22])
        # Building the obstacle 24
        self.obstacle24 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle24 = [self.canvas_window.coords(self.obstacle24)[0] + 3,
         self.canvas_window.coords(self.obstacle24)[1] + 3,
         self.canvas_window.coords(self.obstacle24)[2] - 3,
         self.canvas_window.coords(self.obstacle24)[3] - 3]

        # Obstacle 25
        # Defining the center of obstacle 25
        obstacle25_center = self.o + np.array([pixels * 18, pixels * 22])
        # Building the obstacle 25
        self.obstacle25 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle25 = [self.canvas_window.coords(self.obstacle25)[0] + 3,
         self.canvas_window.coords(self.obstacle25)[1] + 3,
         self.canvas_window.coords(self.obstacle25)[2] - 3,
         self.canvas_window.coords(self.obstacle25)[3] - 3]

        # Obstacle 26
        # Defining the center of obstacle 26
        obstacle26_center = self.o + np.array([pixels * 18, pixels * 21])
        # Building the obstacle 26
        self.obstacle26 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle26 = [self.canvas_window.coords(self.obstacle26)[0] + 3,
         self.canvas_window.coords(self.obstacle26)[1] + 3,
         self.canvas_window.coords(self.obstacle26)[2] - 3,
         self.canvas_window.coords(self.obstacle26)[3] - 3]

        # Obstacle 27
        # Defining the center of obstacle 27
        obstacle27_center = self.o + np.array([pixels * 18, pixels * 20])
        # Building the obstacle 27
        self.obstacle27 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle27 = [self.canvas_window.coords(self.obstacle27)[0] + 3,
         self.canvas_window.coords(self.obstacle27)[1] + 3,
         self.canvas_window.coords(self.obstacle27)[2] - 3,
         self.canvas_window.coords(self.obstacle27)[3] - 3]

        # Obstacle 28
        # Defining the center of obstacle 28
        obstacle28_center = self.o + np.array([pixels * 18, pixels * 19])
        # Building the obstacle 28
        self.obstacle28 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle28 = [self.canvas_window.coords(self.obstacle28)[0] + 3,
         self.canvas_window.coords(self.obstacle28)[1] + 3,
         self.canvas_window.coords(self.obstacle28)[2] - 3,
         self.canvas_window.coords(self.obstacle28)[3] - 3]

        # Obstacle 29
        # Defining the center of obstacle 29
        obstacle29_center = self.o + np.array([pixels * 18, pixels * 18])
        # Building the obstacle 29
        self.obstacle29 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle29 = [self.canvas_window.coords(self.obstacle29)[0] + 3,
         self.canvas_window.coords(self.obstacle29)[1] + 3,
         self.canvas_window.coords(self.obstacle29)[2] - 3,
         self.canvas_window.coords(self.obstacle29)[3] - 3]

        # Obstacle 30
        # Defining the center of obstacle 30
        obstacle30_center = self.o + np.array([pixels * 19, pixels * 18])
        # Building the obstacle 30
        self.obstacle30 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle30 = [self.canvas_window.coords(self.obstacle30)[0] + 3,
         self.canvas_window.coords(self.obstacle30)[1] + 3,
         self.canvas_window.coords(self.obstacle30)[2] - 3,
         self.canvas_window.coords(self.obstacle30)[3] - 3]

        # Obstacle 31
        # Defining the center of obstacle 31
        obstacle31_center = self.o + np.array([pixels * 20, pixels * 18])
        # Building the obstacle 31
        self.obstacle31 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle31 = [self.canvas_window.coords(self.obstacle31)[0] + 3,
         self.canvas_window.coords(self.obstacle31)[1] + 3,
         self.canvas_window.coords(self.obstacle31)[2] - 3,
         self.canvas_window.coords(self.obstacle31)[3] - 3]

        # Obstacle 32
        # Defining the center of obstacle 32
        obstacle32_center = self.o + np.array([pixels * 11, pixels * 6])
        # Building the obstacle 32
        self.obstacle32 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle32 = [self.canvas_window.coords(self.obstacle32)[0] + 3,
         self.canvas_window.coords(self.obstacle32)[1] + 3,
         self.canvas_window.coords(self.obstacle32)[2] - 3,
         self.canvas_window.coords(self.obstacle32)[3] - 3]

        # Obstacle 33
        # Defining the center of obstacle 33
        obstacle33_center = self.o + np.array([pixels * 12, pixels * 6])
        # Building the obstacle 33
        self.obstacle33 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle33 = [self.canvas_window.coords(self.obstacle33)[0] + 3,
         self.canvas_window.coords(self.obstacle33)[1] + 3,
         self.canvas_window.coords(self.obstacle33)[2] - 3,
         self.canvas_window.coords(self.obstacle33)[3] - 3]

        # Obstacle 34
        # Defining the center of obstacle 34
        obstacle34_center = self.o + np.array([pixels * 13, pixels * 6])
        # Building the obstacle 34
        self.obstacle34 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle34 = [self.canvas_window.coords(self.obstacle34)[0] + 3,
         self.canvas_window.coords(self.obstacle34)[1] + 3,
         self.canvas_window.coords(self.obstacle34)[2] - 3,
         self.canvas_window.coords(self.obstacle34)[3] - 3]

        # Obstacle 35
        # Defining the center of obstacle 35
        obstacle35_center = self.o + np.array([pixels * 14, pixels * 6])
        # Building the obstacle 35
        self.obstacle35 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle35 = [self.canvas_window.coords(self.obstacle35)[0] + 3,
         self.canvas_window.coords(self.obstacle35)[1] + 3,
         self.canvas_window.coords(self.obstacle35)[2] - 3,
         self.canvas_window.coords(self.obstacle35)[3] - 3]

        # Obstacle 36
        # Defining the center of obstacle 36
        obstacle36_center = self.o + np.array([pixels * 14, pixels * 7])
        # Building the obstacle 36
        self.obstacle36 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle36 = [self.canvas_window.coords(self.obstacle36)[0] + 3,
         self.canvas_window.coords(self.obstacle36)[1] + 3,
         self.canvas_window.coords(self.obstacle36)[2] - 3,
         self.canvas_window.coords(self.obstacle36)[3] - 3]

        # Obstacle 37
        # Defining the center of obstacle 37
        obstacle37_center = self.o + np.array([pixels * 14, pixels * 5])
        # Building the obstacle 37
        self.obstacle37 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle37 = [self.canvas_window.coords(self.obstacle37)[0] + 3,
         self.canvas_window.coords(self.obstacle37)[1] + 3,
         self.canvas_window.coords(self.obstacle37)[2] - 3,
         self.canvas_window.coords(self.obstacle37)[3] - 3]

        # Obstacle 38
        # Defining the center of obstacle 38
        obstacle38_center = self.o + np.array([pixels * 20, pixels])
        # Building the obstacle 38
        self.obstacle38 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle38 = [self.canvas_window.coords(self.obstacle38)[0] + 3,
         self.canvas_window.coords(self.obstacle38)[1] + 3,
         self.canvas_window.coords(self.obstacle38)[2] - 3,
         self.canvas_window.coords(self.obstacle38)[3] - 3]

        # Obstacle 39
        # Defining the center of obstacle 39
        obstacle39_center = self.o + np.array([pixels * 20, pixels * 2])
        # Building the obstacle 39
        self.obstacle39 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle39 = [self.canvas_window.coords(self.obstacle39)[0] + 3,
         self.canvas_window.coords(self.obstacle39)[1] + 3,
         self.canvas_window.coords(self.obstacle39)[2] - 3,
         self.canvas_window.coords(self.obstacle39)[3] - 3]

        # Obstacle 40
        # Defining the center of obstacle 40
        obstacle40_center = self.o + np.array([pixels * 20, pixels * 3])
        # Building the obstacle 40
        self.obstacle40 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle40 = [self.canvas_window.coords(self.obstacle40)[0] + 3,
         self.canvas_window.coords(self.obstacle40)[1] + 3,
         self.canvas_window.coords(self.obstacle40)[2] - 3,
         self.canvas_window.coords(self.obstacle40)[3] - 3]

        # Obstacle 41
        # Defining the center of obstacle 41
        obstacle41_center = self.o + np.array([pixels * 20, pixels * 4])
        # Building the obstacle 41
        self.obstacle41 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle41 = [self.canvas_window.coords(self.obstacle41)[0] + 3,
         self.canvas_window.coords(self.obstacle41)[1] + 3,
         self.canvas_window.coords(self.obstacle41)[2] - 3,
         self.canvas_window.coords(self.obstacle41)[3] - 3]

        # Obstacle 42
        # Defining the center of obstacle 42
        obstacle42_center = self.o + np.array([pixels * 21, pixels * 4])
        # Building the obstacle 42
        self.obstacle42 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle42 = [self.canvas_window.coords(self.obstacle42)[0] + 3,
         self.canvas_window.coords(self.obstacle42)[1] + 3,
         self.canvas_window.coords(self.obstacle42)[2] - 3,
         self.canvas_window.coords(self.obstacle42)[3] - 3]

        # Obstacle 43
        # Defining the center of obstacle 43
        obstacle43_center = self.o + np.array([pixels * 19, pixels * 4])
        # Building the obstacle 43
        self.obstacle43 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle43 = [self.canvas_window.coords(self.obstacle43)[0] + 3,
         self.canvas_window.coords(self.obstacle43)[1] + 3,
         self.canvas_window.coords(self.obstacle43)[2] - 3,
         self.canvas_window.coords(self.obstacle43)[3] - 3]

        # Obstacle 44
        # Defining the center of obstacle 44
        obstacle44_center = self.o + np.array([pixels * 17, pixels * 10])
        # Building the obstacle 44
        self.obstacle44 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle44 = [self.canvas_window.coords(self.obstacle44)[0] + 3,
         self.canvas_window.coords(self.obstacle44)[1] + 3,
         self.canvas_window.coords(self.obstacle44)[2] - 3,
         self.canvas_window.coords(self.obstacle44)[3] - 3]

        # Obstacle 45
        # Defining the center of obstacle 45
        obstacle45_center = self.o + np.array([pixels * 18, pixels * 10])
        # Building the obstacle 45
        self.obstacle45 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle45 = [self.canvas_window.coords(self.obstacle45)[0] + 3,
         self.canvas_window.coords(self.obstacle45)[1] + 3,
         self.canvas_window.coords(self.obstacle45)[2] - 3,
         self.canvas_window.coords(self.obstacle45)[3] - 3]

        # Obstacle 46
        # Defining the center of obstacle 46
        obstacle46_center = self.o + np.array([pixels * 19, pixels * 10])
        # Building the obstacle 46
        self.obstacle46 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle46 = [self.canvas_window.coords(self.obstacle46)[0] + 3,
         self.canvas_window.coords(self.obstacle46)[1] + 3,
         self.canvas_window.coords(self.obstacle46)[2] - 3,
         self.canvas_window.coords(self.obstacle46)[3] - 3]

        # Obstacle 47
        # Defining the center of obstacle 47
        obstacle47_center = self.o + np.array([pixels * 19, pixels * 9])
        # Building the obstacle 47
        self.obstacle47 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle47 = [self.canvas_window.coords(self.obstacle47)[0] + 3,
         self.canvas_window.coords(self.obstacle47)[1] + 3,
         self.canvas_window.coords(self.obstacle47)[2] - 3,
         self.canvas_window.coords(self.obstacle47)[3] - 3]

        # Obstacle 48
        # Defining the center of obstacle 48
        obstacle48_center = self.o + np.array([pixels * 19, pixels * 8])
        # Building the obstacle 48
        self.obstacle48 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle48 = [self.canvas_window.coords(self.obstacle48)[0] + 3,
         self.canvas_window.coords(self.obstacle48)[1] + 3,
         self.canvas_window.coords(self.obstacle48)[2] - 3,
         self.canvas_window.coords(self.obstacle48)[3] - 3]

        # Obstacle 49
        # Defining the center of obstacle 49
        obstacle49_center = self.o + np.array([pixels * 11, pixels * 23])
        # Building the obstacle 49
        self.obstacle49 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle49 = [self.canvas_window.coords(self.obstacle49)[0] + 3,
         self.canvas_window.coords(self.obstacle49)[1] + 3,
         self.canvas_window.coords(self.obstacle49)[2] - 3,
         self.canvas_window.coords(self.obstacle49)[3] - 3]

        # Obstacle 50
        # Defining the center of obstacle 50
        obstacle50_center = self.o + np.array([pixels * 10, pixels * 23])
        # Building the obstacle 50
        self.obstacle50 = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_obstacle50 = [self.canvas_window.coords(self.obstacle50)[0] + 3,
         self.canvas_window.coords(self.obstacle50)[1] + 3,
         self.canvas_window.coords(self.obstacle50)[2] - 3,
         self.canvas_window.coords(self.obstacle50)[3] - 3]

        # Creating an agent of Mobile Robot - red point
        self.agent = self.canvas_window.create_oval(
            self.o[0] - 7, self.o[1] - 7,
            self.o[0] + 7, self.o[1] + 7,
            outline='#1962FF', fill='#1962FF')

        # Final Point - yellow point
        flag_center = self.o + np.array([pixels * 20, pixels * 20])
        # Building the flag
        self.flag = self.canvas_window.create_rectangle(
        ...
        # In order to fit the coordinates of the agent
        self.coords_flag = [self.canvas_window.coords(self.flag)[0] + 3,
                            self.canvas_window.coords(self.flag)[1] + 3,
                            self.canvas_window.coords(self.flag)[2] - 3,
                            self.canvas_window.coords(self.flag)[3] - 3]
    ...
    def reset(self):
        ...
        # Updating agent
        self.canvas_window.delete(self.agent)
        self.agent = self.canvas_window.create_oval(
            self.o[0] - 7, self.o[1] - 7,
            self.o[0] + 7, self.o[1] + 7,
        ...
        # Return observation
        return self.canvas_window.coords(self.agent)

    # Function to get the next observation and reward by doing next step
    def step(self, action):
        # Current state of the agent
        state = self.canvas_window.coords(self.agent)
        ...
        self.canvas_window.move(self.agent, base_action[0], base_action[1])

        # Writing in the dictionary coordinates of found route
        self.d[self.i] = self.canvas_window.coords(self.agent)
    ...
    def final(self):
        ...
        self.initial_point = self.canvas_window.create_oval(
